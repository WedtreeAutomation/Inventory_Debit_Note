...
def initialize_session_state():
    ...
    session_vars = {
        ...
        'approved_lots': [],
        'rejected_lots': [],
        'processed_lots': {},  # Dictionary for processed lots
        'excel_data': None,
        'selected_damaged_lots': [],
        'select_all_damaged': False,
        'lot_details': {},
        'lot_po_mapping': {},
        'failed_processing': {},
        'widget_key_counter': 0  # Add key counter for widget stability
    }
...
# Utility functions for stable widget management
# Utility functions for stable widget management - BULLETPROOF VERSION
def create_stable_widget_key(base_name):
    """Create stable widget keys that work across deployments"""
    return f"{base_name}_{st.session_state.widget_key_counter}"

def handle_bulk_selection_ui(pending_lots):
    """Bulletproof bulk selection UI that always works"""
    
    # Initialize states
    if "select_all_damaged_state" not in st.session_state:
        st.session_state.select_all_damaged_state = False
    ...
    with col1:
        # Select All checkbox - SIMPLIFIED
        select_all_changed = False
        new_select_all_state = st.checkbox(
            f"🔘 Select All ({len(pending_lots)} items)",
            value=st.session_state.select_all_damaged_state,
            key=create_stable_widget_key("select_all_simple")
        )
        
        if new_select_all_state != st.session_state.select_all_damaged_state:
            st.session_state.select_all_damaged_state = new_select_all_state
            select_all_changed = True
    ...
    # Multi-select with manual synchronization
    if st.session_state.select_all_damaged_state:
    ...
    selected_lots = st.multiselect(
        ...
        key=create_stable_widget_key("damaged_lots_simple"),
        help="Choose individual items or use 'Select All' above"
    )
    
    # Manual synchronization
    if not select_all_changed:  # Only update if select all didn't change
        if selected_lots != st.session_state.selected_damaged_lots:
            ...
            if set(selected_lots) == set(pending_lots) and pending_lots:
                st.session_state.select_all_damaged_state = True
            else:
                st.session_state.select_all_damaged_state = False
    ...
    return st.session_state.selected_damaged_lots
...
def safe_rerun():
    """Safe rerun with small delay to ensure state consistency"""
    time.sleep(0.1)
    st.rerun()
...
def setup_logging():
    """Setup comprehensive logging for inventory processing"""
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler(f"inventory_processing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
            logging.StreamHandler()
        ]
    )
    return logging.getLogger(__name__)

# Add this function to create detailed process logs
def create_processing_log(non_damaged, damaged, approved, rejected, processed, failed_processing=None):
    """Create a comprehensive log of all processing activities"""
    if failed_processing is None:
        failed_processing = {}
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        # Summary sheet
        summary_data = {
            'Processing Stage': ['Total Lots Checked', 'Non-Damaged Lots', 'Damaged Lots', 
        ...
        # Detailed processing log sheet
        processing_log_data = []
        
        # Add non-damaged items
        for item in non_damaged:
            details = item.get('details', {})
            po_details = details.get('po_details', {}) if details else {}
            
            processing_log_data.append({
                'Lot/Serial': item['lot'],
                'Status': 'Non-Damaged',
                'Location': item['location'],
                'PO Number': po_details.get('po_number', 'N/A') if po_details else 'N/A',
                'Processing Stage': 'Inventory Check',
                'Result': 'Success',
                'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'Notes': 'Item not in damage stock'
            })
        ...
        for item in damaged:
            ...
            if lot in processed:
                ...
                status = 'Rejected'
            elif lot in failed_processing:
                status = 'Failed Processing'
            else:
                status = 'Damaged - Pending'
            ...
            processing_log_data.append({
                ...
                'PO Number': po_details.get('po_number', 'N/A') if po_details else 'N/A',
                'Processing Stage': 'Inventory Check',
                'Result': 'Success' if status != 'Failed Processing' else 'Failed',
                'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'Notes': failed_processing.get(lot, '') if status == 'Failed Processing' else ''
            })
        
        # Add processed returns with details
        for lot, details in processed.items():
            processing_log_data.append({
                ...
                'Processing Stage': 'Return Processing',
                'Result': 'Success',
                'Timestamp': details.get('timestamp', datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
                'Notes': f"Return reference: {details.get('returned_reference', 'N/A')}"
            })
        
        # Add failed processing attempts
        for lot, error in failed_processing.items():
            processing_log_data.append({
                'Lot/Serial': lot,
                'Status': 'Failed Processing',
                'Location': 'Unknown',
                ...
                'Result': 'Failed',
                'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'Notes': str(error)[:500]  # Limit error message length
            })
        
        processing_log_df = pd.DataFrame(processing_log_data)
        processing_log_df.to_excel(writer, sheet_name='Detailed Processing Log', index=False)
        
        # Failed items sheet
        if failed_processing:
            failed_data = [{'Lot/Serial': lot, 'Error': error} for lot, error in failed_processing.items()]
            failed_df = pd.DataFrame(failed_data)
            failed_df.to_excel(writer, sheet_name='Failed Items', index=False)
...
def check_inventory(lot_serials):
    ...
    not_in_damage_stock = []
    in_damage_stock = []
    failed_lots = {}  # Track lots that failed processing
    
    # Initialize logger
    logger = setup_logging()
    logger.info(f"Starting inventory check for {len(lot_serials)} lots")
        ...
        try:
            ...
            if quant_records:
                ...
                if not found_in_damage:
                    ...
                    logger.info(f"Lot {lot}: Not in damage stock")
                else:
                    ...
                    logger.info(f"Lot {lot}: Found in damage stock")
                ...
                logger.warning(f"Lot {lot}: Not found in stock.quant")
                
        except Exception as e:
            error_msg = f"Error processing lot {lot}: {str(e)}"
            logger.error(error_msg)
            failed_lots[lot] = error_msg
            # Add to not found as well to maintain count
            not_in_damage_stock.append({
                'lot': lot, 
                'location': f"Error: {str(e)[:100]}",
    ...
    logger.info(f"Inventory check completed: {len(in_damage_stock)} damaged, {len(not_in_damage_stock)} non-damaged, {len(failed_lots)} failed")
    
    # Store failed lots in session state for reporting
    st.session_state.failed_processing = failed_lots
...
def process_product_return(lot_serials):
    ...
    results = {}
    failed_processing = {}  # Track failed items

    # Initialize logger
    logger = setup_logging()
    logger.info(f"Starting return processing for {len(unique_lots)} lots")
    ...
    for lot in unique_lots:
        try:
            processed += 1
            progress_bar.progress(processed / total_lots)
            status_text.text(f"Preparing {processed} of {total_lots} lots for return...")
            
            po_number = get_po_for_lot(lot)
            if po_number.startswith("Error:") or po_number == "Not Found":
                error_msg = f"Cannot process return: {po_number}"
                results[lot] = {'success': False, 'message': error_msg}
                failed_processing[lot] = error_msg
                logger.error(f"Lot {lot}: {error_msg}")
                continue
            ...
            if not move_lines:
                error_msg = f"No move line found for lot: {lot}"
                results[lot] = {'success': False, 'message': error_msg}
                failed_processing[lot] = error_msg
                logger.error(f"Lot {lot}: {error_msg}")
                continue
            ...
            logger.info(f"Lot {lot}: Prepared for return processing")

        except Exception as e:
            error_msg = f"Error preparing lot: {str(e)}"
            results[lot] = {'success': False, 'message': error_msg}
            failed_processing[lot] = error_msg
            logger.error(f"Lot {lot}: {error_msg}")
    ...
    try:
        ...
        error_msg = f"Error getting company ID: {str(e)}"
        logger.error(error_msg)
    ...
    try:
        ...
        if not damage_location:
            error_msg = f"Damage location 'Damge/Stock' not found in Odoo!"
            logger.error(error_msg)
            return False, error_msg
        damage_location_id = damage_location[0]['id']
    except Exception as e:
        error_msg = f"Error finding damage location: {str(e)}"
        logger.error(error_msg)
        return False, error_msg

    # Process each (PO, product) group
    group_count = len(lot_groups)
    current_group = 0
    
    for (po_number, product_id), lots in lot_groups.items():
        try:
            current_group += 1
            status_text.text(f"Processing group {current_group} of {group_count} ({len(lots)} lots)...")
            ...
            if not new_picking_id:
                error_msg = "No return picking created"
                for lot in lots:
                    results[lot] = {'success': False, 'message': error_msg}
                    failed_processing[lot] = error_msg
                    logger.error(f"Lot {lot}: {error_msg}")
            ...
            for lot in lots:
                ...
                logger.info(f"Lot {lot}: Successfully processed return")
    ...
    # Store failed processing in session state
    st.session_state.failed_processing.update(failed_processing)
    ...
    logger.info(f"Return processing completed: {success_count} successful, {failure_count} failed")
            ...
            if isinstance(st.session_state.inventory_results, dict) and 'non_damaged' in st.session_state.inventory_results:
                ...
                if damaged:
                    ...
                    if pending_lots:
                        ...
                        # Use the bulletproof selection UI
                        selected_lots = handle_bulk_selection_ui(pending_lots)
                        
                        if selected_lots:
                            st.info(f"💡 {len(selected_lots)} item(s) selected for action")
                            ...
                            with col1:
                                if st.button(
                                    f"✅ Approve Selected ({len(selected_lots)})",
                                    type="primary", 
                                    use_container_width=True,
                                    key=create_stable_widget_key("approve_selected")
                                ):
                                    ...
                                    st.success(f"✅ Approved {len(selected_lots)} items for return!")
                                    reset_selection_state()
                                    time.sleep(1)
                                    st.rerun()
                            ...
                            with col2:
                                if st.button(
                                    f"❌ Reject Selected ({len(selected_lots)})",
                                    use_container_width=True,
                                    key=create_stable_widget_key("reject_selected")
                                ):
                                    for lot in selected_lots:
                                    ...
                                    st.warning(f"❌ Rejected {len(selected_lots)} items!")
                                    reset_selection_state()
                                    time.sleep(1)
                                    st.rerun()
                            ...
                            with col3:
                                if st.button(
                                    "🔄 Clear Selection", 
                                    use_container_width=True,
                                    key=create_stable_widget_key("clear_selection")
                                ):
                                    reset_selection_state()
                                    st.rerun()
                    else:
                ...
                st.download_button(
                    ...
                    data=create_processing_log(
                        non_damaged, 
                        damaged, 
                        st.session_state.approved_lots,
                        st.session_state.rejected_lots,
                        st.session_state.processed_lots,
                        st.session_state.failed_processing
                    ),
                    file_name=f"processing_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True
                )
